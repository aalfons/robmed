...
fit_mediation.default <- function(object, x, y, m, covariates = NULL,
  ...
  if (method == "regression") {
    ...
    # check control object for robust regression
    if (robust == "MM" && is.null(control)) {
      control <- MM_reg_control(...)
    }
    if (robust == "median" && is.null(control)) {
...
reg_fit_mediation <- function(data, x, y, m, covariates = character(),
                              ...
                              fit_yx = TRUE, control = MM_reg_control()) {
  ...
  if (have_robust) {
    ...
    if (robust == "MM") {
      ...
      if (p_m == 1L) {
        fit_mx <- rq_fit(predictors_mx, data[, m], tau = 0.5, control = control)
      } else {
        # obtain list of models
        if (model == "serial") {
          fit_mx <- mapply(function(m_j, predictors) {
            rq_fit(predictors, data[, m_j], tau = 0.5, control = control)
          }, m, predictors_mx, SIMPLIFY = FALSE, USE.NAMES = FALSE)
        } else {
          fit_mx <- lapply(m, function(m_j) {
            rq_fit(predictors_mx, data[, m_j], tau = 0.5, control = control)
          })
        }
        # add names
        names(fit_mx) <- m
      }
      fit_ymx <- rq_fit(predictors_ymx, data[, y], tau = 0.5, control = control)
    } else {
      # shouldn't happen
  ...
  if (have_robust) result$control <- control
