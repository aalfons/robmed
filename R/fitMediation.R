...
#'
#' \code{fit_mediation} is a wrapper function for compatibility with the
...
#' \code{fit_mediation} but optional otherwise.
...
#' @param \dots  additional arguments to be passed down.  In particular, those
#' can be used to specify tuning parameters directly instead of via
#' \code{control}.
#'
...
fit_mediation <- function(data, x, y, m, covariates = NULL, ...) {
  ...
  # Moreover, is this function relevant for the tidyverse?  For practical use,
  # only test_mediation() is useful.
  fitMediation(x, y, m, covariates = covariates, data = data, ...)
}
