...
#' \code{\link[robustbase]{lmrob.control}}.  If \code{robust} is \code{TRUE} 
...
fitMediation <- function(x, y, m, data, method = c("regression", "covariance"), 
  ...
  if(robust && missing(control)) {
    if(method == "regression") control <- lmrob.control(...)
    else control <- covHuber.control(...)
  }
  ## estimate effects
