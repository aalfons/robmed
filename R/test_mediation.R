...
#     indices_d <- unlist(mapply("[", index_list$fit_mx[-1L], j_list,
#                                USE.NAMES = FALSE), use.names = FALSE)
#     d <- colMeans(bootstrap$t[, indices_d, drop = FALSE], na.rm = TRUE)
#   }
...
#       # FIXME: we may need to set the 'statistic' component to NULL, otherwise
#       #        otherwise something weird could be going on when computing the
#       #        BCa confidence intervals
#       # I checked this, and it's not necessary. There is no jacknife involved
#       # in computing the BCa bootstrap intervals, and the process is completely
#       # deterministic for our bootstrap objects.  This also means that we can
#       # rewrite the bootstrap procedure such that only the models are fitted
#       # inside the bootstrap function, which will greatly simplify the code.
#       # But it also means that we have to treat the covariance fits in the
#       # bootstrap differently, which should not be a huge deal.
...
boot_test_mediation <- function(fit,
  ...
  if (inherits(fit, "reg_fit_mediation")) {
    ...
    if (family == "gaussian") {
      ...
      boot_total <- extract_boot_total(index_list$total, bootstrap = bootstrap)
      if (all(is.na(boot_total))) total <- rep.int(NA_real_, p_x)
      else total <- colMeans(boot_total, na.rm = TRUE)
    }
    ...
    if (nr_indirect == 1L) {
      ...
      ci <- lapply(seq_len(nr_indirect), function(j) {
        confint(bootstrap_indirect, parm = j, level = level,
                alternative = alternative, type = type)
      })
      ci <- do.call(rbind, ci)
