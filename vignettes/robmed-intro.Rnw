...
and the same holds for the corresponding ordinary least-squares (OLS) estimates.
...
for \proglang{SPSS} \citep{SPSS}, which links to the \proglang{R} package
\pkg{robmed} and allows to use its main functionality through a GUI from
within \proglang{SPSS}.  Interested readers can obtain the extension bundle
from \url{https://github.com/aalfons/ROBMED-RSPSS}.
