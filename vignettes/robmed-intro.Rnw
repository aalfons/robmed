...
In the social, behavioral, and medical sciences, \emph{mediation analysis} is a
popular statistical technique for studying how an \emph{independent variable}
affects a \emph{dependent variable} indirectly through an intervening variable
called a \emph{mediator}.
...
random error terms.  The coefficients $c$ and $c'$ are called the \emph{direct
effect} and \emph{total effect}, respectively, of $X$ on $Y$.  The product of
coefficients $ab$ is called the \emph{indirect effect} of $X$ on $Y$ and
constitutes the main parameter of interest in mediation analysis.  Under the
usual assumption of independent and normally distributed error terms $e_{1}$,
$e_{2}$ and $e_{3}$, it holds that $c' = ab + c$ \citep[e.g.,][]{mackinnon95},
and the same holds for the corresponding OLS estimates.

\begin{figure}[t]
\begin{center}
\includegraphics[width=0.5\textwidth]{figures/simple_mediation}
\end{center}
...
\code{sem} \citep{sem} and \pkg{lavaan} \citep{lavaan} can be used for
...
In the \emph{parallel multiple mediator model}, an independent variable $X$ is
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.6\textwidth]{figures/parallel_mediation}
\end{center}
...
% In contrast, the \emph{serial multiple mediator model} allows that the mediators
% influence each other in a sequential manner.
A distinctive feature of the \emph{serial multiple mediator model} is that the
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.7\textwidth]{figures/serial_mediation}
\end{center}
...
Instead of having multiple mediators, one can also allow for \emph{multiple
...
\begin{figure}[t!]
\begin{center}
\includegraphics[width=0.525\textwidth]{figures/multiple_mediation}
\end{center}
...
\emph{control variables} should be added to all regression equations of a
...
many authors advocating to use the bootstrap with OLS estimation of the
coefficients in the mediation model \citep{bollen90, shrout02, mackinnon04,
preacher04, preacher08}.
...
also includes the loss function and weight function from ordinary least-squares
(OLS) regression for comparison.

\begin{figure}[t]
{\centering \includegraphics[width=0.975\textwidth]{figures/figure-loss-1}}
\caption[Loss function (\emph{left}) and corresponding weight function (\emph{right}) for OLS regression and the robust MM-estimator of regression]{Loss function (\emph{left}) and corresponding weight function (\emph{right}) for OLS regression and the robust MM-estimator of regression.}\label{fig:loss}
\end{figure}
...
explanatory variables.  Some \proglang{R} package for mediation analysis
require the user to specify one formula for each equation (e.g.,
\pkg{mediation}), which can be tedious and prone to mistakes, in particular for
models with multiple mediators and multiple independent variables or control
variables.  Other packages do not provide a formula interface at all (e.g.,
\pkg{psych} or \pkg{MBESS}), despite formulas being the standard way of
describing models in \proglang{R}.
...
which implements various methods for the \emph{estimation} of a mediation
model, and \code{test_mediation()}, which performs \emph{statistical tests}
on the indirect effects in the mediation model.
...
\code{robustbase} \citep{robustbase}, but it is shortened as the output is
...
in a similar way as the output of the widely-used \proglang{SPSS} macro
\code{PROCESS} \citep{hayes18}, which implements the OLS bootstrap test for
conditional process models such as the mediation model.  The intention is to
facilitate the use of package \pkg{robmed} for users of the \code{PROCESS}
macro.  While \code{PROCESS} constructs a bootstrap confidence interval for
the indirect effect, it reports estimates on the original data and the usual
...
for \proglang{SPSS}, which links to the \proglang{R} package \pkg{robmed}
and allows to use its main functionality through a GUI from within
\proglang{SPSS}.  Interested readers can obtain the extension bundle from
...
and its dependencies \pkg{boot} version~\Sexpr{packageVersion("boot")}
\citep{boot}, \pkg{ggplot2} version~\Sexpr{packageVersion("ggplot2")}
\citep{wickham16}, and \pkg{robustbase}
version~\Sexpr{packageVersion("robustbase")} \citep{robustbase}.
