...
<<loss, echo=FALSE, fig.width=8.5, fig.height = 3.5, out.width="0.975\\textwidth", fig.cap="Loss function (\\emph{left}) and corresponding weight function (\\emph{right}) for OLS regression and the robust MM-estimator of regression.">>=
ggplot() +
