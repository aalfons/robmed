context("bootstrap test with regression fit: simple mediation, no covariates")
...
ctrl <- reg_control(efficiency = 0.95)  # control parameters for MM-regression
...
classes <- c(robust = "summary_lmrob", median = "summary_rq",
             standard = "summary_lm", student = "summary_lmse",
             select = "summary_lm")


## stuff needed to check correctness
coef_names <- c("a", "b", "Total", "Direct", "Indirect")
mx_names <- yx_names <- c("(Intercept)", "X")
...
for (method in methods) {
  ...
  test_that("summary has correct structure", {
    ...
    # diagnostic plot only for robust bootstrap test
    if (method == "robust") {
  ...
  for (i in 1:nrow(combinations)) {
    ...
    test_that("model summary has correct structure", {
      ...
      # model summary
      expect_s3_class(model_summary, classes[method])
      # regression standard error, R-squared, and F-test
      if (method %in% c("robust", "standard", "select")) {
        ...
        df_test_boot <- model_summary$F_test$df
        if (method == "robust") {
          expect_identical(df_test_boot[1], df1[i])
          expect_identical(df_test_boot[2], Inf)
        } else {
          expect_identical(df_test_boot[1], as.integer(df1[i]))
          expect_identical(df_test_boot[2], model_summary$s$df)
        }
      } else {
        # regression standard error
