...
fit_list <- list(
  robust = {
    ...
    fit_mediation(test_data, x = "X", y = "Y", m = c("M1", "M2"),
                  model = "serial", method = "regression",
                  robust = TRUE, efficiency = efficiency)
  },
  median = {
    fit_mediation(test_data, x = "X", y = "Y", m = c("M1", "M2"),
                  model = "serial", method = "regression",
...
## correct values
coef_names <- c("a_M1", "a_M2", "b_M1", "b_M2", "d", "Total", "Direct",
...
for (method in methods) {
  ...
  test_that("dimensions are correct", {
    ...
    expect_named(fit$indirect, c("Total", "M1", "M2", "M1->M2"))
  ...
  test_that("values of coefficients are correct", {
    ...
    d <- unname(coef(fit$fit_mx$M2)["M1"])
  ...
  test_that("covariates not implemented", {
    ...
    expect_warning(
      cov_fit <- fit_mediation(test_data, x = "X", y = "Y", m = c("M1", "M2"),
                               model = "serial", method = "covariance",
                               robust = robust)
    )
