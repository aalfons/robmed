...
for (method in methods) {
  ...
  test_that("dimensions are correct", {
    ...
    expect_identical(dim(boot$reps$t), c(as.integer(R), 20L))

  })

  test_that("values of coefficients are correct", {

    expect_equivalent(boot$a, colMeans(boot$reps$t[, c(6, 11)]))
    expect_equivalent(boot$b, colMeans(boot$reps$t[, 15:16]))
    expect_equivalent(boot[["d"]], mean(boot$reps$t[, 10]))
    expect_equivalent(boot$direct, mean(boot$reps$t[, 17]))
    expect_equivalent(boot$indirect, colMeans(boot$reps$t[, 1:4]))
    expect_equivalent(boot$reps$t[, 1], rowSums(boot$reps$t[, 2:4]))
    expect_equivalent(boot$reps$t[, 2], boot$reps$t[, 6] * boot$reps$t[, 15])
    expect_equivalent(boot$reps$t[, 3], boot$reps$t[, 11] * boot$reps$t[, 16])
    expect_equivalent(boot$reps$t[, 4],
                      boot$reps$t[, 6] * boot$reps$t[, 10] * boot$reps$t[, 16])
    # total effect
    expect_equivalent(boot$total, mean(boot$reps$t[, 20]))
    if (method %in% c("robust", "median", "OLS")) {
      expect_equivalent(boot$reps$t[, 20], rowSums(boot$reps$t[, c(1, 17)]))
    }
