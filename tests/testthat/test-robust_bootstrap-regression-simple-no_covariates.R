...
test_that("output has correct structure", {
  ...
  expect_s3_class(foo$fit$fit_mx, "lmrob")
  expect_s3_class(foo$fit$fit_ymx, "lmrob")
...
# test_that("nonrobust regression method yields output of correct structure", {
#   # run bootstrap test
#   out <- test_mediation(test_data, x = "X", y = "Y", m = "M",
#                         test = "boot", R = R, level = level,
#                         method = "regression", robust = FALSE)
#
#   # check for correct classes
#   expect_s3_class(out, "boot_test_mediation")
#   expect_s3_class(out$fit, "reg_fit_mediation")
#   expect_s3_class(out$reps, "boot")
#
#   # check if arguments are correctly passed
#   expect_equal(out$R, R)  # if there are too many outliers this doesn't hold
#   expect_equal(out$level, level)
# })
#
# test_that("robust covariance method yields output of correct structure", {
#   # run bootstrap test
#   out <- test_mediation(test_data, x = "X", y = "Y", m = "M",
#                         test = "boot", R = R, level = level,
...
#   # check for correct classes
#   expect_s3_class(out, "boot_test_mediation")
#   expect_s3_class(out$fit, "cov_fit_mediation")
#   expect_s3_class(out$reps, "boot")
#
#   # check if arguments are correctly passed
#   expect_equal(out$R, R)  # if there are too many outliers this doesn't hold
...
# })
#
# test_that("nonrobust covariance method yields output of correct structure", {
#   # run bootstrap test
#   out <- test_mediation(test_data, x = "X", y = "Y", m = "M",
#                         test = "boot", R = R, level = level,
