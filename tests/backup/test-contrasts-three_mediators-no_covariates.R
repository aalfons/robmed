context("contrasts: three mediators, no covariates")
...
M3 <- rnorm(n)
Y <- b * M1 + c * X + rnorm(n)
test_data <- data.frame(X, Y, M1, M2, M3)
...
# without contrasts
set.seed(seed)
boot_without <- test_mediation(test_data, x = "X", y = "Y",
                               m = c("M1", "M2", "M3"),
...
set.seed(seed)
boot_estimates <- test_mediation(test_data, x = "X", y = "Y",
                                 m = c("M1", "M2", "M3"),
                                 test = "boot", R = R, level = level,
                                 type = "perc", method = "regression",
...
set.seed(seed)
boot_absolute <- test_mediation(test_data, x = "X", y = "Y",
                                m = c("M1", "M2", "M3"),
                                test = "boot", R = R, level = level,
                                type = "perc", method = "regression",
...
indirect_names <- c("Total", "M1", "M2", "M3",
...
test_that("model fit works correctly", {
  ...
  expect_length(fit_estimates$ab, 7L)
  expect_identical(names(fit_estimates$ab), indirect_names)
  expect_length(fit_absolute$ab, 7L)
  expect_identical(names(fit_absolute$ab), indirect_names)

  # contrasts computed correctly
  expect_equivalent(fit_estimates$ab["Contrast1"],
                    fit_estimates$ab["M1"] - fit_estimates$ab["M2"])
  expect_equivalent(fit_estimates$ab["Contrast2"],
                    fit_estimates$ab["M1"] - fit_estimates$ab["M3"])
  expect_equivalent(fit_estimates$ab["Contrast3"],
                    fit_estimates$ab["M2"] - fit_estimates$ab["M3"])
  expect_equivalent(fit_absolute$ab["Contrast1"],
                    abs(fit_absolute$ab["M1"]) - abs(fit_absolute$ab["M2"]))
  expect_equivalent(fit_absolute$ab["Contrast2"],
                    abs(fit_absolute$ab["M1"]) - abs(fit_absolute$ab["M3"]))
  expect_equivalent(fit_absolute$ab["Contrast3"],
                    abs(fit_absolute$ab["M2"]) - abs(fit_absolute$ab["M3"]))
...
test_that("objects returned by setup_xxx_plot() have correct structure", {
  ...
  # check dimensions
  expect_identical(dim(ci_estimates$ci), c(8L, 4L))
  expect_identical(dim(ci_absolute$ci), c(8L, 4L))
  ...
  expect_equivalent(ci_estimates$ci[-(6L:8L), ], ci_without$ci)
  expect_equivalent(ci_absolute$ci[-(6L:8L), ], ci_without$ci)
  ...
  # check dimensions
  expect_identical(dim(ci_p_estimates$ci), c(8L, 5L))
  expect_identical(dim(ci_p_estimates$p_value), c(8L, 3L))
  expect_identical(dim(ci_p_absolute$ci), c(8L, 5L))
  expect_identical(dim(ci_p_absolute$p_value), c(8L, 3L))
  # check that labels are correct
  label_names <- c("Confidence interval", "p-Value")
  expect_identical(ci_p_estimates$ci$Label,
                   factor(rep.int(label_names[1], 8), levels = label_names))
  expect_identical(ci_p_estimates$p_value$Label,
                   factor(rep.int(label_names[2], 8), levels = label_names))
  ...
  # check that values for direct and indirect effects are correct
  expect_equivalent(ci_p_estimates$ci[-(6L:8L), ], ci_p_without$ci)
  expect_equivalent(ci_p_estimates$p_value[-(6L:8L), ], ci_p_without$p_value)
  expect_equivalent(ci_p_absolute$ci[-(6L:8L), ], ci_p_without$ci)
  expect_equivalent(ci_p_absolute$p_value[-(6L:8L), ], ci_p_without$p_value)
  ...
  expect_true("Contrast1" %in% density_estimates$density$Effect)
  expect_true("Contrast2" %in% density_estimates$density$Effect)
  expect_true("Contrast3" %in% density_estimates$density$Effect)
  expect_true("Contrast1" %in% density_absolute$density$Effect)
  expect_true("Contrast2" %in% density_absolute$density$Effect)
  expect_true("Contrast3" %in% density_absolute$density$Effect)
  # check that values for density estimates are correct
  remove_estimates <- grep("Contrast", density_estimates$density$Effect)
  expect_equivalent(density_estimates$density[-remove_estimates, ],
                    density_without$density)
  remove_absolute <- grep("Contrast", density_absolute$density$Effect)
  ...
  # check dimensions
  expect_identical(dim(density_estimates$ci), c(7L, 4L))
  expect_identical(dim(density_absolute$ci), c(7L, 4L))
  # check column names
  ...
  expect_equivalent(density_estimates$ci[-(5L:7L), ], density_without$ci)
  expect_equivalent(density_absolute$ci[-(5L:7L), ], density_without$ci)
